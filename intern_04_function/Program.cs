    ...
    public class Program
    {
        ...
        public static class FileZipBase64
        {
            public static string FilePath_In(string filePath)
            {
                ...
                //檔案大小確認
                int maxFileSizeInBytes = int.Parse(ConfigurationManager.AppSettings["MaxFileSizeInBytes"]);
                ...
                //合法檔案格式
                string allowedFileTypesString = ConfigurationManager.AppSettings["AllowedFileTypes"];
                ...
                //判斷檔案格式
                bool isValidFileType = Array.Exists(allowedFileTypes, type => type.ToLower() == Ext);

                if (!isValidFileType)
                {
                    throw new Exception("格式不合格！");
                }
